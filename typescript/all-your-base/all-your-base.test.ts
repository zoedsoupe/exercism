...
describe('Converter', () => {
  ...
  it('empty list', () => {
    expect( convert([], 2, 10)).toEqual('Input has wrong format');
  });
  ...
  it('multiple zeros', () => {
    expect(convert([0, 0, 0], 10, 2)).toEqual('Input has wrong format');
  });

  it('leading zeros', () => {
    expect(convert([0, 6, 0], 7, 10)).toEqual('Input has wrong format');
  });

  it('negative digit', () => {
    expect(convert([1, -1, 1, 0, 1, 0], 2, 10)).toEqual('Input has wrong format');
  });

  it('invalid positive digit', () => {
    expect(convert([1, 2, 1, 0, 1, 0], 2, 10)).toEqual('Input has wrong format');
  });

  it('first base is one', () => {
    expect(convert([], 1, 10)).toEqual('Wrong input base');
  });

  it('second base is one', () => {
    expect(convert([1, 0, 1, 0, 1, 0], 2, 1)).toEqual('Wrong output base');
  });

  it('first base is zero', () => {
    expect(convert([], 0, 10)).toEqual('Wrong input base');
  });

  it('second base is zero', () => {
    expect(convert([7], 10, 0)).toEqual('Wrong output base');
  });

  it('first base is negative', () => {
    expect(convert([1], -2, 10)).toEqual('Wrong input base');
  });

  it('second base is negative', () => {
    expect(convert([1], 2, -7)).toEqual('Wrong output base');
  });

  it('both bases are negative', () => {
    expect(convert([1], -2, -7)).toEqual('Wrong input base');
  });

  it('wrong output_base base not integer', () => {
    expect(convert([0], 3, 2.5)).toEqual('Wrong output base');
  });
});
